...
public class ExecuteStandingInstructionsTasklet implements Tasklet {
        ...
        for (StandingInstructionData data : instructionData) {
            ...
            if (data.getToAccountType().isLoanAccount()
                ...
                if (data.getInstructionType().isDuesAmoutTransfer()) {
                    final SavingsAccount fromSavingsAccount = this.savingsAccountAssembler.assembleFrom(data.getFromAccount().getId(),
                            false);
                    BigDecimal availableBalance = fromSavingsAccount.getWithdrawableBalance();
    ...
    private boolean transferAmount(final List<Throwable> errors, final AccountTransferDTO accountTransferDTO, final Long instructionId) {
        ...
        try {
            ...
            log.warn("Insufficient balance while transferring funds for standing Instruction id {} from {} to {}. Skipping transfer.",
            ...
            log.warn("Account blocked while transferring funds for standing Instruction id {} from {} to {}. Skipping transfer.",
                    instructionId, accountTransferDTO.getFromAccountId(), accountTransferDTO.getToAccountId());
